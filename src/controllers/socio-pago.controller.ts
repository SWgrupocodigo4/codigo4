...
import { insertarSocioPagoSchema, actualizarSocioPagoSchema } from "../validators/socio-pago.schema";

export const insertarSocioPago = async (req: Request, res: Response) => {
    try {
        console.log('insertarSocioPago');
        const { error } = insertarSocioPagoSchema.validate(req.body);
        if (error) {
...
export const actualizarSocioPago = async (req: Request, res: Response) => {
    try {
        const { idSocioPago } = req.params;
        const { error } = actualizarSocioPagoSchema.validate(req.body);
        if (error) {
            res.status(400).json(BaseResponse.error(error.message, 400));
            return;
        }
        ...
        const actualizarSocioPago: SocioPago = await socioPagoService.actualizarSocioPago(Number(idSocioPago), socioPago);
        res.json(BaseResponse.success(actualizarSocioPago, Message.ACTUALIZADO_OK));
    } catch (error) {
        console.error(error);
        res.status(500).json(BaseResponse.error(error.message));
    }
